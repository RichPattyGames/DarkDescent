...
if(distance_to_object(obj_playerHome) <= 0)
{
	playerRecoil = 20;
}
...
if(walking_up or walking_down or walking_right or walking_left)
{
	//xdirectionif (walking_left or walking_right)
	...
	if (walking_up or walking_down)
	{
		...
		yDirection = walking_down - walking_up;
	
	    // If there is not a wall walk_speed away
	    if (!place_meeting(x, y + (player_speed*yDirection), obj_collision_block))
	    {
	        // Move walk_speed
	        y += player_speed*yDirection;
	    ...
	    {
	        // So move 1 pixel up until you are next to the wall
	        while (!place_meeting(x , y + yDirection, obj_collision_block))
	        {
	            y = yDirection;
	        }
	    }
